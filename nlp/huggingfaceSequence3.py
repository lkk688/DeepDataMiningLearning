#created on 11/24, plan to add summarization
...
def loadmodel(model_checkpoint, mycache_dir="", pretrained="", hpc=True, unfreezename=""):
    if hpc==True:
...
def loaddata(args, USE_HPC):
    ...
    return raw_datasets, text_column, target_column
...
if __name__ == "__main__":
    ...
    model, tokenizer, starting_epoch = loadmodel(model_checkpoint, mycache_dir=mycache_dir, pretrained=args.pretrained, hpc=USE_HPC, unfreezename=args.unfreezename)
    ...
    raw_datasets, text_column, target_column = loaddata(args, USE_HPC)
    ...
    def translationpreprocess_function(examples):
        if task == "translation":
            inputs = [ex[text_column] for ex in examples["translation"]]
            targets = [ex[target_column] for ex in examples["translation"]]
        elif task == "summarization":
