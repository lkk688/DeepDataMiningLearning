...
class KittiCalibration(object):
    ...
    def project_rect_to_velo(self, pts_3d_rect, cameraid=0):
        """ Input: nx3 points in rect camera coord.
            Output: nx3 points in velodyne coord.
        """
    ...
    # ===========================
    # Newly added
    def project_cam3d_to_image(self, pts_3d_rect, cameraid=0):
        """ Input: nx3 points in rect camera coord.
            Output: nx2 points in image coord.
        """
        pts_2d, depth = self.project_rect_to_image(pts_3d_rect)
        # pts_3d_rect = self.cart2hom(pts_3d_rect)#nx3 to nx4 by pending 1
        # pts_2d = np.dot(pts_3d_rect, np.transpose(self.P[cameraid]))  # nx3
        # pts_2d[:, 0] /= pts_2d[:, 2] #normalize the depth pts_2d[:, 2]
        # pts_2d[:, 1] /= pts_2d[:, 2]
        return pts_2d, depth
    #project_cam3d_to_image in WaymoCalibration is similar, but with more paramaters
    ...
    def project_velo_to_image(self, pts_3d_velo, cameraid=0): #add cameraid to match with WaymoCalibration
        """ Input: nx3 points in velodyne coord.
            Output: nx2 points in image2 coord.
        """
        if pts_3d_velo.shape[1]!=3: #only get xyz
            print("points dimension is not 3")
            pts_3d_velo=pts_3d_velo[:,0:3]
