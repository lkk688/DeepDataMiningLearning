...
class NuScenesLoader(BaseLoader):
    def __init__(self, dataroot, version, pipeline_conf=None, max_samples=5):
        ...
        if pipeline_conf is None:
            self.sweeps_num = 5
            self.final_dim = (256, 704) # H, W
            self.resize_scale = 0.48
        else:
            self.sweeps_num = pipeline_conf['sweeps_num']
            self.final_dim = pipeline_conf['final_dim']
            self.resize_scale = pipeline_conf['resize_scale']
    ...
    def load_imgs(self, sample):
        ...
        for c_name in cams:
            ...
            # Apply dynamic crop (Center Crop)
            sx, sy = (nW - target_w)//2, (nH - target_h)//2
            ...
            # cam2img.append(K)
            # Update Intrinsic Matrix for Resize and Crop
            K[0]*=scale_factor; K[1]*=scale_factor; K[0,2]-=sx; K[1,2]-=sy
            cam2img.append(K)
    ...
    def __iter__(self):
        for i, sample in enumerate(self.nusc.sample):
            ...
            metainfo = {
                ...
                'img_shape': [self.final_dim]*6, # Dynamic shape
...
def get_loader(dataset_type, dataroot, version, pipeline_conf, max_samples):
    print(f"Initializing Loader for {dataset_type}...")
    if dataset_type == 'nuscenes': 
        return NuScenesLoader(dataroot, version, pipeline_conf, max_samples)
    elif dataset_type == 'kitti': 
        return KITTILoader(dataroot, max_samples)
    else: 
        return AnyLoader(dataroot, max_samples)
...
def extract_pipeline_params(cfg):
    """
    Extracts inference parameters from the test_pipeline in the config.
    Returns a dictionary of parameters with safe defaults.
    """
    # Defaults (matching your previous hardcoded values)
    params = {
        'sweeps_num': 5,
    ...
    # MMEngine config objects act like dicts, but let's be safe
    ...
    for step in pipeline:
        ...
        # 1. Get Sweeps
        if t_type == 'LoadPointsFromMultiSweeps':
            params['sweeps_num'] = step.get('sweeps_num', params['sweeps_num'])
            
        # 2. Get Image Augmentation Parameters
        elif t_type == 'ImageAug3D':
            # final_dim in config is usually [H, W]
            f_dim = step.get('final_dim', None)
            if f_dim: 
                params['final_dim'] = tuple(f_dim)
            
            # resize_lim in test pipeline is usually [scale, scale] (fixed)
            r_lim = step.get('resize_lim', None)
            if r_lim: 
                # Take the first value as the fixed inference scale
                params['resize_scale'] = r_lim[0]
                
    print(f"Loaded Pipeline Params: {params}")
    return params
...
def main():
    ...
    # 2. Extract Pipeline Params
    pipeline_conf = extract_pipeline_params(cfg)
    
    # 3. System Info & Validation
...
python simple_infer_allv2b.py \
    ...
    --out-dir ./bevfusion_infer_results_v \
