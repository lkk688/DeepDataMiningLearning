...
model = dict(
    ...
    view_transform=dict(
        ...
        in_channels=256,
        out_channels=128,            # 64 for lower memory/bandwidth
        image_size=[256, 704],
    ...
    fusion_layer=dict(
        type='ConvFuser',
        in_channels=[128, 256],      # [camera_out, lidar_out]
        out_channels=256
    ),
...
if voxel_painting_on:
    ...
    pts_voxel_encoder = dict(
        ...
        cam_pool='max',            # 'avg' (smoother) or 'max' (more robust)
        img_feat_out=64,           # dimension of per-voxel image descriptor
...
optim_wrapper = dict(
    ...
    paramwise_cfg=dict(custom_keys={
        ...
        'img_backbone.norm3': dict(lr_mult=0.05, decay_mult=0.0)
    }),
...
for h in custom_hooks:
    if h['type']=='FreezeExceptHook':
        h['freeze_norm'] = False

load_from = '/data/rnd-liu/MyRepo/mmdetection3d/work_dirs/mybevfusion7_newv2/epoch_2.pth'  # ← EDIT ME
...
work_dir = 'work_dirs/mybevfusion7_newv3'