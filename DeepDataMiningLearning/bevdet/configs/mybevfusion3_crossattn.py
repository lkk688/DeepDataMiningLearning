...
# 同时加载 BEVFusion 和你新加的 CrossAttn LSS
custom_imports = dict(
    imports=[
        'projects.bevdet.bevfusion',      # 负责注册 BEVFusion 模型等
        'projects.bevdet.cross_attn_lss'  # 你新增的 CrossAttnLSSTransform
    ],
...
model = dict(
    ...
    view_transform=dict(
        type='CrossAttnLSSTransform', #DepthLSSTransform',
        in_channels=256,
        out_channels=80,
        image_size=[256, 704],
        feature_size=[32, 88],
        ...
        zbound=[-5.0, 5.0, 10.0],     # 用于生成高度锚点范围（不必与原值完全一致）
        dbound=[1.0, 60.0, 0.5],      # 仅用于推导默认K时的参考，这里保留即可
        downsample=2,
        # cross-attn 参数（可按需微调）
        num_z=2,                      # K：高度锚点个数（显存∝K）
        use_cam_embed=True,
        attn_chunk=2048,               # 分块大小，显存和速度的trade-off
        debug=False            # <- turn off prints
    ),
