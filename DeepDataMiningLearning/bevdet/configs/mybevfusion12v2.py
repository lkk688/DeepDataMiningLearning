...
model = dict(
    ...
    view_transform=dict(
        ...
        out_channels=160,           # camera BEV channels kept small for bandwidth
        image_size=[256, 704],
        ...
        num_z=6,
        use_cam_embed=True,
        attn_chunk=16384,           # H100-friendly; 16384 if memory allows
    ...
    fusion_layer=dict(
        type='ConvFuser', #'ConvFuser',
        in_channels=[160,256], ##[128, 256],
        out_channels=256
    ),
    ...
    # CHANGE: stronger aux supervision (focal+dice) and slightly higher weight
...
if voxel_painting_on:
    ...
    pts_voxel_encoder = dict(
        ...
        img_feat_out=96,            # per-voxel image descriptor dim before gating
...
if use_rpf_lidar:
    ...
    model.update(dict(
        pts_neck=dict(
            ...
            blocks_per_stage=3,            # tiny, stable; increase if headroom
            with_residual=True,
            use_cbam=True, ca_reduction=16, sa_kernel=7,
        )
    ))
...
param_scheduler = [
    ...
    dict(type='CosineAnnealingLR', begin=0, T_max=10, end=10, by_epoch=True, eta_min_ratio=1e-4, convert_to_iter_based=True),
    dict(type='CosineAnnealingMomentum', eta_min=0.85/0.95, begin=0, end=2.4, by_epoch=True, convert_to_iter_based=True),
    dict(type='CosineAnnealingMomentum', eta_min=1, begin=2.4, end=10, by_epoch=True, convert_to_iter_based=True)
]
...
#load_from = '/data/rnd-liu/MyRepo/mmdetection3d/work_dirs/mybevfusion12/epoch_6.pth'
load_from = '/data/rnd-liu/MyRepo/mmdetection3d/work_dirs/mybevfusion12v2/epoch_3.pth'
