...
# ---------------------------
# 1) DUMMY INPUT BUILDER
#    Make inputs that your forward_dummy will actually accept.
# ---------------------------
def build_dummy_inputs_for_forward_dummy(model, device="cuda", B=1, Ncams=6, H=256, W=704):
    """
    Try several common signatures for mmdet3d models:
      - forward_dummy(img)                      -> img: (B, 3, H, W)
      - forward_dummy(imgs)                     -> imgs: (B, N, 3, H, W)
      - forward_dummy(batch_inputs_dict)        -> {'imgs': (B,N,3,H,W)}
      - forward_dummy(imgs=None, **kwargs)      -> pass kwargs with 'imgs'
    Returns: a tuple (args, kwargs) that runs without error OR raises.
    """
    if not hasattr(model, "forward_dummy"):
        raise RuntimeError("Model has no forward_dummy; add one or wrap your forward for FLOPs.")

    img_sv  = torch.randn(B, 3, H, W, device=device)
    imgs_mv = torch.randn(B, Ncams, 3, H, W, device=device)
    batch_inputs = {"imgs": imgs_mv}

    # Try simplest positional single-view
    try:
        model.forward_dummy(img_sv)
        return (img_sv,), {}
    except Exception:
        pass

    # Try positional multi-view
    try:
        model.forward_dummy(imgs_mv)
        return (imgs_mv,), {}
    except Exception:
        pass
    ...
    # Try dict positional
    try:
        model.forward_dummy(batch_inputs)
        return (batch_inputs,), {}
    except Exception:
        pass

    # Try kwargs with 'imgs'
    try:
        model.forward_dummy(imgs=imgs_mv)
        return (), {"imgs": imgs_mv}
    except Exception:
        pass
    ...
    # Try kwargs with 'img'
    try:
        model.forward_dummy(img=img_sv)
        return (), {"img": img_sv}
    except Exception:
        pass

    # If you also have LiDAR, you can add a 'points' placeholder as needed:
    # points = [torch.zeros(1,5, device=device)]  # (N,5)
    # try: model.forward_dummy({"imgs": imgs_mv, "points": points}); return ({"imgs": imgs_mv, "points": points}, {})

    raise RuntimeError("Could not find a working signature for forward_dummy(). "
                       "Open forward_dummy in your model and see what it expects.")
...
# ---------------------------
def report_model_complexity(model, device="cuda", B=1, Ncams=6, H=256, W=704):
    # Params
    total, train = count_params(model)
    print(f"Params (total/trainable): {human_params(total)} / {human_params(train)}")
    print(f"~Model size (fp32): {total*4/1e6:.1f} MB")

    # Build inputs that *actually* work for your forward_dummy
    args, kwargs = build_dummy_inputs_for_forward_dummy(model, device=device, B=B, Ncams=Ncams, H=H, W=W)

    # Try mmengine first
    flops = None
    try:
        flops = flops_mmengine_safe(model, args, kwargs, device=device)
        print(f"FLOPs (mmengine): {human_flops(flops)}")
        if flops > 0:
            return
    except Exception as e:
        print(f"[mmengine FLOPs failed] {e}")

    # Fallback: run forward_dummy once with hooks for Conv/Linear FLOPs
    def run_fn(*a, **k):
        # call forward_dummy directly with the working signature we found
        return model.forward_dummy(*a, **k)

    hook_flops = estimate_flops_by_hooks(model.eval().to(device), run_fn, *args, **kwargs)
    print(f"FLOPs (hook estimate, Conv/Linear only): {human_flops(hook_flops)}")
    if hook_flops == 0:
        print("Still 0? Your forward_dummy may be empty or uses only unsupported/custom ops.\n"
              "Either implement forward_dummy to run the main compute path, or install a profiler:\n"
              "  pip install fvcore  # FlopCountAnalysis\n"
              "  pip install thop    # MACs (≈ FLOPs/2)")
...
def main():
    ...
    #report_model_complexity(model, device=str(args.device), B=1, Ncams=6, H=256, W=704)
    ...
    with torch.inference_mode(), torch.autocast(device_type='cuda' if 'cuda' in args.device else 'cpu',
        ...
        for lidar_path, img_paths, metainfo, basename in iter_nuscenes_samples(
            ...
            # imgs: [B,N,3,256,704]
            imgs_ = load_and_stack_images(img_paths, size_hw=(256, 704))  # [N,3,H,W], float32
