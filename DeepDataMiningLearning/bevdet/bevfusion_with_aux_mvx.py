...
class BEVFusionWithAux(BEVFusion):
    ...
    Minimal change: add a boolean switch `voxel_painting_on`. When True, we
    register a tiny *pre-forward hook* on `view_transform` to stash the FPN
    feature (its input) and the batch metas into a thread-local context so a
    VFE wrapper can read them to perform VoxelPainting. When False, nothing
    changes versus the original pipeline.
    """


    def __init__(self, aux_cfg: Optional[Dict] = None, 
                 voxel_painting_on: bool = False, **kwargs):
        super().__init__(**kwargs)
        self.voxel_painting_on = bool(voxel_painting_on)  # << NEW: save switch
    ...
    def loss(self, inputs: Dict, data_samples: List, **kwargs) -> Dict:
        ...
        # << NEW: only when voxel painting is enabled, stash VT input (FPN feat)
        h_pre = None
        if self.voxel_painting_on:
            batch_metas = [item.metainfo for item in data_samples]

            def _vt_pre_hook(module, in_tup):
                # in_tup[0] is the FPN feature fed into VT in this branch:
                # shape is typically [B, Nc, C, Hf, Wf]
                fpn_feats = in_tup[0]
                set_painting_context(fpn_feats, batch_metas)

            h_pre = self.view_transform.register_forward_pre_hook(_vt_pre_hook)
