...
class TaskAlignedAssigner(nn.Module):
    ...
    def get_box_metrics(self, pd_scores, pd_bboxes, gt_labels, gt_bboxes, mask_gt):
        ...
        # Clamp gt_labels to valid class indices to prevent CUDA indexing errors
        ind[1] = torch.clamp(ind[1], 0, pd_scores.shape[-1] - 1)
    ...
    def get_targets(self, gt_labels, gt_bboxes, target_gt_idx, fg_mask):
        ...
        # Ensure target_labels are within valid range for scatter operation
    ...
    def select_highest_overlaps(mask_pos, overlaps, n_max_boxes):
        ...
        Args:
            mask_pos (torch.Tensor): Positive mask with shape (b, n_max_boxes, h*w).
            overlaps (torch.Tensor): IoU overlaps with shape (b, n_max_boxes, h*w).
        ...
        Returns:
            ...
            fg_mask (torch.Tensor): A boolean tensor of shape (b, h*w) indicating the positive
                              (foreground) anchor points.
            mask_pos (torch.Tensor): Updated positive mask with shape (b, n_max_boxes, h*w).
        """
