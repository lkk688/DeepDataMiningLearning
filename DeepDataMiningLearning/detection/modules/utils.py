#ref: https://github.com/lkk688/myyolov7/blob/main/utils/general.py
# YOLOR general utils
...
LOGGER = logging.getLogger("myyolo")  # define globally (used in train.py, val.py, detect.py, etc.)
...
def extract_filename(file_path, without_extension=True):
    """
    Extracts the filename from a given file path and removes the file type extension.
...
#xcyc to xmin, ymin, xmax, ymax
def xywh2xyxy(x):
    # Convert nx4 boxes from [x, y, w, h] to [x1, y1, x2, y2] where xy1=top-left, xy2=bottom-right
    y = x.clone() if isinstance(x, torch.Tensor) else np.copy(x)
    y[:, 0] = x[:, 0] - x[:, 2] / 2  # top left x = xcenter-w/2
    y[:, 1] = x[:, 1] - x[:, 3] / 2  # top left y = ycenter-h/2
    y[:, 2] = x[:, 0] + x[:, 2] / 2  # bottom right x = xcenter+w/2
    y[:, 3] = x[:, 1] + x[:, 3] / 2  # bottom right y = ycenter+h/2
    return y
...
#ref: https://github.com/ultralytics/ultralytics/blob/main/ultralytics/nn/modules/utils.py
...
#ref: https://github.com/ultralytics/ultralytics/blob/main/ultralytics/models/utils/ops.py
...
#ref: https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/ops.py
def xywh2xyxy(x):
    """
